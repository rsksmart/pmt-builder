...
const buildPMT = (leaves, filteredHash) => {
    ...
    return {
        ...
        hex: `${reverseHex(lpad(leaves.length.toString(16), 2))}${lpad(hashes.length.toString(16), 8)}${hashes.map(reverseHex).join('')}${lpad(flags.length.toString(16), 2)}${flags.toString('hex')}`
    };
};
